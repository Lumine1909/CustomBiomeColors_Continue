...
public class ServerDataHandler_1_20_5 implements ServerDataHandler<Biome, Holder<Biome>, ResourceKey<Biome>> {
    ...
    public BiomeAccessor<Biome, Holder<Biome>, ResourceKey<Biome>> getBiomeFromKey(BiomeKey biomeKey) {
        ...
        return new BiomeAccessor_1_20_5(BIOME_REGISTRY.getHolder(ResourceKey.create(Registries.BIOME, new ResourceLocation(biomeKey.namespace(), biomeKey.value()))).orElseThrow());
    }
    ...
    public boolean hasBiome(BiomeKey biomeKey) {
        return BIOME_REGISTRY.getHolder(ResourceKey.create(Registries.BIOME, new ResourceLocation(biomeKey.namespace(), biomeKey.value()))).isPresent();
    }
    ...
    public BiomeAccessor<Biome, Holder<Biome>, ResourceKey<Biome>> createCustomBiome(BiomeData biomeData, boolean register) {
        Holder<Biome> holder = BIOME_REGISTRY.getHolder(ResourceKey.create(
            Registries.BIOME,
            new ResourceLocation(biomeData.baseBiomeKey().namespace(), biomeData.baseBiomeKey().value())
        )).orElse(PLAINS);
        ...
        ResourceKey<Biome> resourceKey = ResourceKey.create(Registries.BIOME, new ResourceLocation(biomeKey.namespace(), biomeKey.value()));
        ...
        return register
            ? new BiomeAccessor_1_20_5(this.registerBiome(holder, customBiome, resourceKey), biomeData)
            : new BiomeAccessor_1_20_5(customBiome, biomeData);
    }
